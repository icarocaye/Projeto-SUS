...
bool apagar_no_rec(no *n)
{
    ...
    apagar_no_rec(n->esq);
    apagar_no_rec(n->dir);
    // Não apaga os dados (paciente/pilha), apenas o nó
    RegistroApagar(n->dados);
    free(n);
    return true;
}
...
void em_ordem(no *raiz)
{
    if(raiz==NULL)
        return;
    em_ordem(raiz->esq);
    printf("\nID: %d\nNome: %s\n",raiz->dados->paciente->id, raiz->dados->paciente->nome);
    em_ordem(raiz->dir);
}
...
Registro* arvore_buscar(int id, no *raiz)
{
    ...
    if(id == raiz_id)
        ...
        return arvore_buscar(id, raiz->esq);
    else // id > raiz_id
        return arvore_buscar(id, raiz->dir);
}
...
no* remover_aux(int id, no *raiz)
{
    ...
    else 
    {
        ...
        for(tmp = raiz->esq; tmp->dir != NULL; tmp = tmp->dir);
...
void arvore_remover(int id, Arvore *arv){
    arv->raiz = remover_aux(id,arv->raiz);
}
bool arvore_inserir(Registro *reg, Arvore *arv)
{
